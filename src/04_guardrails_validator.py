...
class PIIDetector(Validator):
    ...
    def validate(self, value: str, metadata: dict):
        ...
        for pii_type, pattern in self.PII_PATTERNS.items():
            # Tìm tất cả matches trong redacted_text (để xử lý nhiều loại PII)
            matches = re.findall(pattern, redacted_text)

            for match in matches:
                # Thay thế match bằng "[PII_TYPE_REDACTED]" trong redacted_text
                redacted_text = redacted_text.replace(match, f"[{pii_type}_REDACTED]", 1)
                found_pii.append((pii_type, match))

        if found_pii:
            print(f"  ⚠️  Đã redact {len(found_pii)} PII: {[p[0] for p in found_pii]}")
            # Trả về FailResult với fix_value=redacted_text để FIX action hoạt động
            return FailResult(
...
class JSONFormatter(Validator):
    ...
    def _repair(text: str) -> str:
        ...
        # Xóa trailing commas (dùng re.sub)
